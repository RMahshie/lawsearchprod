...
- `app/api/endpoints/query.py`: `/api/query`, `/api/ingest`, `/api/health`, `/api/status`.
...
- Sources/debug chunks are first-class response fields for future citation hover UI.

## API Behavior

`QueryRequest` supports `question`, `thinking_speed`, `max_results`, `include_sources`, `debug_chunks`, `divisions_filter`, and OpenAI-only `model_override`.

`QueryResponse` includes final answer, selected divisions, per-division results, sources, optional debug chunks, timing, query id, thinking speed, and model used.
